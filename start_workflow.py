...
def main():
    ...
    gmail_proc = start_process("Gmail Watcher", "gmail_watcher_live.py")
    update_dashboard_status("Gmail Watcher", "Running")

    if not gmail_only:
        # 2. File Watcher
        file_proc = start_process("File Watcher", "file_watcher_live.py")
        update_dashboard_status("File Watcher", "Running")

        # 3. WhatsApp Watcher (optional - needs browser)
        if not no_whatsapp:
            wa_proc = start_process("WhatsApp Watcher", "whatsapp_watcher_live.py")
        ...
        orchestrator_proc = start_process("Orchestrator", "orchestrator_live.py")
        update_dashboard_status("Orchestrator", "Running")

        # 5. Approval Flow Watcher
        approval_proc = start_process("Approval Watcher", "approval_watcher.py")

        # 6. Scheduler (CEO Briefing, Subscription Audit, Daily Summary)
        scheduler_proc = start_process("Scheduler", "scheduler_live.py")

        # 7. MCP Servers (Social Media, Odoo, Email, Calendar, Slack, Payment)
        mcp_proc = start_process("MCP Servers", "start_mcp_servers.py")
    ...
    while running:
        ...
        for name, proc in processes:
            if proc.poll() is not None:
                ...
                script_map = {
                    "Gmail Watcher": "gmail_watcher_live.py",
                    "File Watcher": "file_watcher_live.py",
                    "WhatsApp Watcher": "whatsapp_watcher_live.py",
                    "Orchestrator": "orchestrator_live.py",
                    "Approval Watcher": "approval_watcher.py",
                    "Scheduler": "scheduler_live.py",
                    "MCP Servers": "start_mcp_servers.py",
                }
