#!/usr/bin/env python3
"""
Test script for orchestrator-engine skill
"""
...
def test_skill_structure():
    """Test that the orchestrator-engine skill has the correct structure"""
    skill_path = Path("E:/hackathon 0 qwen/Personal-AI-Employee/.claude/skills/orchestrator-engine")

    print("Testing orchestrator-engine skill structure...")
    ...
    if skill_md.exists():
        ...
        with open(skill_md, 'r', encoding='utf-8') as f:
            content = f.read()
            if "orchestrator-engine" in content and "Claude Code" in content:
...
def test_imports():
    """Test that the orchestrator scripts can be imported without errors"""
    print("\nTesting script imports...")

    # Add the scripts directory to Python path
    scripts_path = "E:/hackathon 0 qwen/Personal-AI-Employee/.claude/skills/orchestrator-engine/scripts"
    sys.path.insert(0, scripts_path)

    try:
        # Test importing orchestrate_claude
        import orchestrate_claude
        print("[OK] orchestrate_claude.py imports successfully")
    except ImportError as e:
        print(f"[ERROR] Failed to import orchestrate_claude.py: {e}")
    except Exception as e:
        print(f"[ERROR] Error importing orchestrate_claude.py: {e}")
...
def main():
    """Run all tests"""
    print("=" * 60)
    print("Orchestrator Engine Skill - Test Suite")
    print("=" * 60)
